...
static void KTH71_SPI_TransmitReceive(uint8_t *pTxData, uint8_t *pRxData, uint8_t txLen, uint8_t rxLen)
{
		SPI_DMA_HalfDuplex_TxStartAsync(pTxData, txLen);
		while(!SPI_DMA_IsDone());  
		SPI_DMA_HalfDuplex_RxStartAsync(pRxData, rxLen);
		while(!SPI_DMA_IsDone());   
}
void KTH71_UnlockReg(void)
{
  uint8_t dataSend[4] = {0x20, 0x24, 0x01, 0x01};
  SPI_DMA_HalfDuplex_TxStartAsync(dataSend, 4);
	while(!SPI_DMA_IsDone()); 
}
...
void KTH71_WriteRegToMTP(void)
{
  ...
  SPI_DMA_HalfDuplex_TxStartAsync(dataSend, 3);
	while(!SPI_DMA_IsDone()); 
  LL_mDelay(400);
}
